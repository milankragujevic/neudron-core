...
void Bayang_send_packet(u8 bind) {
	...
	if (bind) {
		...
		packet[0] = 0xa5;
		packet[1] = 0xf7; // normal mode 0xf7, expert mode 0xfa
		packet[2] = GET_FLAG(AUX2, BAYANG_FLAG_FLIP) | GET_FLAG(AUX5, BAYANG_FLAG_HEADLESS)| GET_FLAG(AUX6, BAYANG_FLAG_RTH) | GET_FLAG(AUX3, BAYANG_FLAG_SNAPSHOT) | GET_FLAG(AUX4, BAYANG_FLAG_VIDEO);
		packet[3] = GET_FLAG(AUX1, BAYANG_FLAG_INVERT);
		chanval.value = map(ppm[AILERON], PPM_MIN, PPM_MAX, 0, 0x3ff);
		packet[4] = chanval.bytes.msb + DYNTRIM(chanval.value);
		packet[5] = chanval.bytes.lsb;
		chanval.value = map(ppm[ELEVATOR], PPM_MIN, PPM_MAX, 0, 0x3ff);
		packet[6] = chanval.bytes.msb + DYNTRIM(chanval.value);
		packet[7] = chanval.bytes.lsb;
		chanval.value = map(ppm[THROTTLE], PPM_MIN, PPM_MAX, 0, 0x3ff);
		packet[8] = chanval.bytes.msb + 0x7c;
		packet[9] = chanval.bytes.lsb;
		chanval.value = map(ppm[RUDDER], PPM_MIN, PPM_MAX, 0, 0x3ff);
